...
fileprivate extension Binding where Value == ContentView7.Category {
  func noFlicker ( option: Binding < ContentView7.Option > ) -> Self {
...
extension Binding where Value == ContentView7.Option {
  var noFlicker: Self {
    Binding ( get: { self.wrappedValue } , set: { nv in
      guard self.wrappedValue != nv else { return }
